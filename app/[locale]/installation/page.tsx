...
export default async function InstallationPage({ params }: PageProps) {
  ...
  const hasUnpublishedWorks = installation.works.some((work) => !work.description);
    ...
    <article>
      <PageShell full>
        <PatchPageHeader
          title={installation.title}
          statusNote={
            hasUnpublishedWorks || tbaArtists.length > 0
              ? dict.status.updatesInProgress
              : undefined
          }
        />

        {installation.works.length > 0 ? (
          <section>
            ...
            <ul className="-mx-4 sm:-mx-6 lg:-mx-8">
              {installation.works.map((work) => {
                const isPublished = Boolean(work.description);
                ...
                return (
                  <li
                    ...
                    className="list-row border-t border-pntrsw-deep/10 first:border-t-0"
                  >
                    <div className="flex w-full flex-col gap-2 px-4 py-5 sm:flex-row sm:items-start sm:justify-between sm:gap-6 sm:px-6 lg:px-8">
                      <h3 className="type-headline text-xl leading-snug text-pntrsw-body/60 sm:flex-1">
                        {work.title}
                      </h3>
                      <ArtistCredits
                        artists={work.artists}
